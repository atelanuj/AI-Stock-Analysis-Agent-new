...
def screen_funds(request:FundScreenRequest)->dict:
    ...
    def work(identifier):
        ...
        return {"identifier":data.get("identifier",identifier),"name":data.get("scheme_name"),"market":data.get("market"),"quote_type":data.get("quote_type"),"category":data.get("scheme_category"),"score":a["score"],"score_breakdown":a.get("score_breakdown"),"rating":a["rating"],"current_nav":data.get("current_nav"),"currency":data.get("currency"),"return_1d_pct":r.get("1D"),"return_1y_pct":r.get("1Y"),"return_3y_annualized_pct":r.get("3Y"),"return_5y_annualized_pct":r.get("5Y"),"sharpe_ratio":risk.get("sharpe_ratio"),"sortino_ratio":risk.get("sortino_ratio"),"max_drawdown_pct":risk.get("max_drawdown_pct"),"annualized_volatility_pct":risk.get("annualized_volatility_pct"),"expense_ratio_pct":_expense_pct(data.get("expense_ratio")),"aum":data.get("total_assets"),"alpha_vs_benchmark_pct":bench.get("alpha_vs_benchmark_pct"),"tracking_error_pct":bench.get("tracking_error_pct")}
    ...
    return {"asset_type":"FUND","market":request.market,"requested_scan_count":request.scan_count,"evaluated":len(candidates),"matched":len(valid),"returned":len(returned),"top":returned,"errors":errors,"note":"V7 reports scan, match and return counts separately. Fund ranking uses fund-specific return, consistency, risk, cost, drawdown and benchmark metrics. Click a returned row in the UI for the full fund analysis."}
